...
| KOPS_CLUSTER_NAME                                  | Cluster base hostname (E.g. `us-west-2.testing.ryanjarv.sh`)                                    |
...
| KOPS_STATE_STORE                                   | S3 Bucket that will be used to store the cluster state (E.g. `us-west-2.testing.ryanjarv.sh`)   |
...
```
✓   (rjsh-testing-admin) kops ⨠  kops validate cluster
Validating cluster us-west-2.testing.ryanjarv.sh
...
Your cluster us-west-2.testing.ryanjarv.sh is ready
```
...
```
✓   (rjsh-testing-admin) kops ⨠  kubectl get nodes
...
```
✓   (rjsh-testing-admin) backing-services ⨠  kubectl get pods --all-namespaces
...
3. Run `geodesic` shell (`testing.ryanjarv.sh`), assume role (`assume-role`) and change directory to `/conf/kops` folder
